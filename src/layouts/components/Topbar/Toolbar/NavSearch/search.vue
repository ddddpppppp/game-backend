...
const { generateI18nTitle: baseGenerateI18nTitle } = useMenu()

// 创建一个包装函数来处理国际化
function generateI18nTitle(titleOrKey: string | (() => any)) {
  const title = baseGenerateI18nTitle(titleOrKey)
  return typeof title === 'string' && t(title) !== title ? t(title) : title
}
